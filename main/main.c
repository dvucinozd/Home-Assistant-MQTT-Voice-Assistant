...
static void mqtt_update_music_state(music_state_t state, int current_track,
                                    int total_tracks);
static void mqtt_publish_telemetry(void);
static void mqtt_metrics_task(void *arg);
static bool get_wifi_rssi(int *out_rssi);
static void ota_progress_handler(ota_state_t state, int progress,
                                 const char *message);
static void led_ready_task(void *arg);
static void led_ready_task(void *arg);
static void sdcard_release_for_wifi_fallback(void);
static void music_state_callback(music_state_t state, int current_track,
                                 int total_tracks);
...
static void music_control_task(void *arg) {
  ...
  if (cmd == MUSIC_CMD_PLAY) {
    ...
    // Wait for WWD to stop and release I2S/codec.
    // Increased wait time to ensure pipeline task processes the STOP command
    bool stopped = false;
    for (int i = 0; i < 40; i++) {
    ...
    if (!stopped) {
      ESP_LOGW(TAG, "Voice pipeline did not stop in time, forcing ahead...");
    } else {
      vTaskDelay(pdMS_TO_TICKS(200)); // Extra grace period for I2S cleanup
    }

    // Try to initialize if not ready (e.g. if SD was slow or previous init
    // failed)
    if (!local_music_player_is_initialized() && bsp_sdcard) {
      ESP_LOGI(TAG, "Music player not initialized, attempting init now...");
      local_music_player_init();
      local_music_player_register_callback(music_state_callback);
    }

    if (local_music_player_is_initialized()) {
      if (local_music_player_play() != ESP_OK) {
        ESP_LOGE(TAG, "Failed to start music player");
        // Optional: Play error beep here if we had access to beep_tone
      }
    } else {
      ESP_LOGE(TAG,
               "Cannot play music: Player not initialized (SD not mounted?)");
    }
...
static void mqtt_publish_telemetry(void) {
  ...
  mqtt_ha_update_sensor("ota_status",
                        ota_state_to_string(ota_update_get_state()));
  snprintf(buf, sizeof(buf), "%d", ota_update_get_progress());
  mqtt_ha_update_sensor("ota_progress", buf);

  mqtt_ha_update_sensor("sd_card_status",
                        bsp_sdcard ? "MOUNTED" : "NOT_MOUNTED");
  ...
  mqtt_ha_update_number("agc_target_level",
                        (float)va_control_get_agc_target_level());
...
static void post_connect_task(void *arg) {
  ...
  ESP_LOGI(TAG, "Network Connected: %s (IP: %s)",
           network_manager_type_to_string(type), ip_str);
...
static void mqtt_diagnostic_dump_callback(const char *entity_id,
                                          const char *payload) {
  (void)entity_id;
  (void)payload;
...
static void mqtt_music_play_callback(const char *entity_id,
  ...
  if (music_control_task_handle == NULL) {
    xTaskCreate(music_control_task, "music_ctl", 4096,
                (void *)(uintptr_t)MUSIC_CMD_PLAY, 5,
...
static void mqtt_music_stop_callback(const char *entity_id,
  ...
  if (music_control_task_handle == NULL) {
    xTaskCreate(music_control_task, "music_ctl", 4096,
                (void *)(uintptr_t)MUSIC_CMD_STOP, 5,
                &music_control_task_handle);
  }
}
...
static void mqtt_output_volume_callback(const char *entity_id,
                                        const char *payload) {
  (void)entity_id;
  if (!payload)
    return;
  ...
  if (!audio_hw_ready) {
    ESP_LOGW(TAG,
             "Output volume set requested but audio hardware is not ready");
    return;
  }
...
static void mqtt_agc_enabled_callback(const char *entity_id,
                                      const char *payload) {
  (void)entity_id;
  if (!payload)
    return;
...
static void mqtt_led_indicator_callback(const char *entity_id,
                                        const char *payload) {
  (void)entity_id;
  if (!payload)
    return;
...
static void mqtt_setup_task(void *arg) {
  ...
  mqtt_ha_register_switch("auto_gain_control", "Auto Gain Control",
                          mqtt_agc_enabled_callback);
  mqtt_ha_register_switch("led_status_indicator", "LED Status Indicator",
                          mqtt_led_indicator_callback);
  ...
  mqtt_ha_register_button("diagnostic_dump", "Diagnostic Dump",
                          mqtt_diagnostic_dump_callback);
  ...
  mqtt_ha_register_sensor("wifi_signal", "WiFi Signal", "dBm",
                          "signal_strength");
  ...
  mqtt_ha_register_sensor("webserial_requests", "WebSerial Requests", NULL,
                          NULL);
  ...
  mqtt_ha_register_number("wwd_detection_threshold", "WWD Detection Threshold",
                          0.5f, 0.95f, 0.01f, "", mqtt_wwd_threshold_callback);

  mqtt_ha_register_text("ota_url_input", "OTA URL", mqtt_ota_url_callback);
  mqtt_ha_register_button("ota_trigger", "Start OTA",
  ...
  mqtt_ha_register_number("vad_threshold", "VAD Threshold", 0, 1000, 10, "",
                          mqtt_vad_threshold_callback);
  mqtt_ha_register_number("vad_silence_ms", "VAD Silence (ms)", 100, 5000, 100,
                          "ms", mqtt_vad_silence_callback);
  mqtt_ha_register_number("vad_min_speech_ms", "VAD Min Speech (ms)", 100, 2000,
                          50, "ms", mqtt_vad_min_speech_callback);
  mqtt_ha_register_number("vad_max_recording_ms", "VAD Max Rec (ms)", 1000,
                          15000, 500, "ms", mqtt_vad_max_recording_callback);
  ...
  // Publish current IP once MQTT is up (covers cases where network connected
  // earlier).
  char ip_str[16];
  if (network_manager_get_ip(ip_str) == ESP_OK) {
    mqtt_ha_update_sensor("ip_address", ip_str);
  ...
  mqtt_ha_update_number("agc_target_level",
                        (float)va_control_get_agc_target_level());
  mqtt_ha_update_number("wwd_detection_threshold",
                        va_control_get_wwd_threshold());

  // Publish initial VAD settings
  mqtt_ha_update_number("vad_threshold", (float)va_control_get_vad_threshold());
  mqtt_ha_update_number("vad_silence_ms",
                        (float)va_control_get_vad_silence_duration_ms());
  mqtt_ha_update_number("vad_min_speech_ms",
                        (float)va_control_get_vad_min_speech_ms());
  mqtt_ha_update_number("vad_max_recording_ms",
                        (float)va_control_get_vad_max_recording_ms());
  ...
  if (metrics_task_handle == NULL) {
    xTaskCreate(mqtt_metrics_task, "mqtt_metrics", 4096, NULL, 4,
                &metrics_task_handle);
  }
...
static void network_event_callback(network_type_t type, bool connected) {
  if (connected) {
    if (post_connect_task_handle == NULL) {
      xTaskCreate(post_connect_task, "net_post", 4096, (void *)(uintptr_t)type,
                  5, &post_connect_task_handle);
    }
...
static void music_state_callback(music_state_t state, int current_track,
                                 int total_tracks) {
  bool is_playing =
      (state == MUSIC_STATE_PLAYING || state == MUSIC_STATE_PAUSED);
...
void app_main(void) {
  ...
  mqtt_ha_config_t mqtt_conf = {
      ...
      // Treat empty strings as "not set" so MQTT auth is truly optional.
      .username =
          (settings.mqtt_username[0] != '\0') ? settings.mqtt_username : NULL,
      .password =
          (settings.mqtt_password[0] != '\0') ? settings.mqtt_password : NULL,
  ...
  if (!safe_mode) {
    ha_client_config_t ha_conf = {.hostname = settings.ha_hostname,
    ...
    // Camera Init (shares I2C bus with audio codec)
    ...
    if (led_ready_task_handle == NULL) {
      xTaskCreate(led_ready_task, "led_ready", 2048, NULL, 2,
                  &led_ready_task_handle);
    }
  } else {
    // Safe Mode Loop
