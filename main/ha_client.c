                                    ...
                                    int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        case WEBSOCKET_EVENT_CONNECTED:
            ...
            esp_websocket_client_send_text(ws_client, auth_msg, strlen(auth_msg), portMAX_DELAY);
        ...
        case WEBSOCKET_EVENT_DISCONNECTED:
            ...
            xEventGroupClearBits(ha_event_group, HA_CONNECTED_BIT | HA_AUTHENTICATED_BIT);
            break;
                        ...
                        if (event_type && event_type->valuestring) {
                            ...
                            if (strcmp(event_type->valuestring, "run-start") == 0) {
                                ESP_LOGI(TAG, "Pipeline started");
                                    ...
                                    if (data_obj) {
                                        cJSON *runner_data = cJSON_GetObjectItem(data_obj, "runner_data");
                                        if (runner_data) {
                                            cJSON *stt_handler = cJSON_GetObjectItem(runner_data, "stt_binary_handler_id");
                                            if (stt_handler && cJSON_IsNumber(stt_handler)) {
                                                stt_binary_handler_id = stt_handler->valueint;
                                                ESP_LOGI(TAG, "STT binary handler ID: %d", stt_binary_handler_id);
                                            }
                                        }
                                    }
                                    ...
                                    if (data_obj) {
                                        cJSON *intent_output = cJSON_GetObjectItem(data_obj, "intent_output");
                                        if (intent_output) {
                                            cJSON *conversation_id = cJSON_GetObjectItem(intent_output, "conversation_id");
                                            ESP_LOGI(TAG, "Intent matched (conv: %s)",
                                                    conversation_id && conversation_id->valuestring ? conversation_id->valuestring : "none");
                                        }
                                    }
                                    ...
                                    ESP_LOGE(TAG, "Pipeline error: %s - %s",
                                    ...
                                    if (speech) {
                                        ...
                                        if (plain) {
                                            ...
                                            if (speech_text && speech_text->valuestring) {
                                                ...
                                                if (conversation_callback) {
                                                    cJSON *conversation_id = cJSON_GetObjectItem(result, "conversation_id");
                                                    const char *conv_id = conversation_id ? conversation_id->valuestring : NULL;
...
static void download_tts_audio(const char *url)
{
    ...
    if (err == ESP_OK) {
        ...
        if (status_code == 200) {
            ESP_LOGI(TAG, "TTS download complete: status=%d, length=%d", status_code, content_length);
...
esp_err_t ha_client_init(void)
{
    ...
    char ws_uri[256];
    if (HA_USE_SSL) {
        snprintf(ws_uri, sizeof(ws_uri), "wss://%s:%d%s", HA_HOSTNAME, HA_PORT, HA_WEBSOCKET_PATH);
    } else {
        snprintf(ws_uri, sizeof(ws_uri), "ws://%s:%d%s", HA_HOSTNAME, HA_PORT, HA_WEBSOCKET_PATH);
    }
    ...
    if (HA_USE_SSL) {
        ...
        // Python equivalent: sslopt={'cert_reqs': ssl.CERT_NONE, 'check_hostname': False}
        ...
        ESP_LOGI(TAG, "SSL/TLS enabled, relying on CONFIG_ESP_TLS_SKIP_SERVER_CERT_VERIFY");
    } else {
    ...
    EventBits_t bits = xEventGroupWaitBits(ha_event_group,
...
{
    return ws_connected && ws_authenticated;
}
...
esp_err_t ha_client_stream_audio(const uint8_t *audio_data, size_t length, const char *conversation_id)
{
    ...
    int ret = esp_websocket_client_send_bin(ws_client, (const char *)frame, 1 + length, portMAX_DELAY);
    free(frame);
    ...
    ESP_LOGD(TAG, "Sent audio chunk: %d bytes (handler_id=%d)", length, stt_binary_handler_id);
    return ESP_OK;
}
...
esp_err_t ha_client_end_audio_stream(void)
{
    ...
    int ret = esp_websocket_client_send_bin(ws_client, (const char *)end_frame, 1, portMAX_DELAY);
