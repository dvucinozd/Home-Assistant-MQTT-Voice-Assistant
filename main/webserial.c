...
static const char *dashboard_html =
    ...
    "4px rgba(0,0,0,0.1);margin-bottom:20px} "
    "button{padding:10px "
    "20px;margin:5px;cursor:pointer;border:none;border-radius:4px;background:#"
    "007bff;color:white} button:hover{background:#0056b3} "
    "input{padding:10px;width:100%;max-width:400px;margin-bottom:10px;border:"
    "1px solid #ddd;border-radius:4px}</style></head>"
    "<body>"
    "<h2>ESP32-P4 Voice Assistant</h2>"
    "<div class='card'><h3>System Status</h3><div "
    "id='status'>Loading...</div><button "
    "onclick='fetchStatus()'>Refresh</button></div>"
    "<div class='card'><h3>Controls</h3><button "
    "onclick=\"doAction('restart')\">Reboot Device</button><button "
    "onclick=\"doAction('wwd_resume')\">Start WWD</button><button "
    "onclick=\"doAction('wwd_stop')\">Stop WWD</button><button "
    "onclick=\"doAction('led_test')\">LED Test</button></div>"
    "<div class='card'><h3>OTA Update</h3><input type='text' id='otaUrl' "
    "placeholder='http://192.168.1.x:8000/firmware.bin'><br><button "
    "onclick='startOta()'>Start Update</button></div>"
    "<div class='card'><h3>Diagnostics</h3><a href='/webserial'><button>View "
    "Real-time Logs</button></a></div>"
    "<script>"
    "function "
    "fetchStatus(){fetch('/api/"
    "status').then(r=>r.json()).then(j=>{document.getElementById('status')."
    "innerText='IP: '+j.ip+' | Uptime: '+j.uptime+'s | WWD Active: "
    "'+(j.wwd?'Yes':'No')})}"
    "function "
    "doAction(cmd){fetch('/api/action',{method:'POST',body:'cmd='+cmd})}"
    "function startOta(){const url=document.getElementById('otaUrl').value; "
    "if(!url){alert('URL is empty');return;} if(confirm('Start OTA update? "
    "Device will "
    "reboot.')){fetch('/api/"
    "ota',{method:'POST',body:'url='+url}).then(r=>r.json()).then(j=>alert(j."
    "ok?'Update started! Check logs.':'Failed to start update'))}}"
...
static const char *webserial_html =
    ...
    "initial-scale=1'>"
    "<style>body{font-family:sans-serif;margin:16px}button{padding:8px "
    "14px;margin:4px}"
    ".paused{background:#dc3545;color:white}.running{background:#28a745;color:"
    "white}</style>"
    "</head><body><h2>System Logs</h2>"
    "<button id='pauseBtn' class='running' onclick='togglePause()'>⏸ "
    "Pause</button>"
    "<button onclick='location.reload()'>Refresh</button>"
    "<button onclick='clearLogs()'>Clear</button>"
    " <a href='/'><button>Back to Dashboard</button></a><hr>"
    "<pre id='c' style='height:70vh;overflow:auto;border:1px solid "
    "#ddd;padding:8px'></pre>"
    "<script>"
    ...
    "paused=!paused;"
    "if(paused){pauseBtn.innerText='▶ Resume';pauseBtn.className='paused';}"
    "else{pauseBtn.innerText='⏸ Pause';pauseBtn.className='running';}"
    "}"
    "function poll(){"
    "if(paused)return;"
    ...
    "if(logEl.innerText.length>20000){logEl.innerText=logEl.innerText.slice(-"
    "20000);}"
    ...
    "}"
    "function "
    "clearLogs(){fetch('/webserial/"
    "clear').then(()=>{logEl.innerText='';lastSeq=0;});}"
...
static esp_err_t logs_handler(httpd_req_t *req) {
  ...
  if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
    if (httpd_query_key_value(query, "since", since_str, sizeof(since_str)) ==
        ESP_OK) {
...
static bool form_get_param(const char *body, const char *key, char *out,
                           size_t out_len) {
