...
static void diag_worker_task(void *arg) {
  ...
  while (1) {
    // Wait for a timer notification to do heavier work outside the timer
    // service task.
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

    if (safe_mode_active) {
      ESP_LOGW(TAG, "Safe mode stable period reached - clearing boot count and "
                    "rebooting");
    } else {
...
static void determine_reset_reason(void) {
  ...
  case ESP_RST_POWERON:
    reset_reason_str = "Power On";
    break;
  case ESP_RST_EXT:
    reset_reason_str = "External Pin";
    break;
  case ESP_RST_SW:
    reset_reason_str = "Software Reset";
    break;
  case ESP_RST_PANIC:
    reset_reason_str = "Crash/Panic";
    break;
  case ESP_RST_INT_WDT:
    reset_reason_str = "Interrupt WDT (Loop)";
    break;
  case ESP_RST_TASK_WDT:
    reset_reason_str = "Task WDT (Hang)";
    break;
  case ESP_RST_WDT:
    reset_reason_str = "Other WDT";
    break;
  case ESP_RST_DEEPSLEEP:
    reset_reason_str = "Deep Sleep";
    break;
  case ESP_RST_BROWNOUT:
    reset_reason_str = "Brownout (Voltage)";
    break;
  case ESP_RST_SDIO:
...
esp_err_t sys_diag_init(void) {
  ...
  if (err != ESP_OK) {
    // NVS might not be ready if main hasn't init it yet, but we usually call
    // this after NVS init. Assuming NVS init is done in main before this.
    return ESP_OK;
  }
  ...
  // Only count crash-like resets; avoid false safe-mode from flashing/manual
  // reset.
  ...
  if (diag_worker_task_handle == NULL) {
    xTaskCreate(diag_worker_task, "diag_worker", 4096, NULL, 5,
                &diag_worker_task_handle);
  }

  if (boot_count >= 3) {
    ESP_LOGW(TAG,
             "BOOT LOOP DETECTED! Safe Mode DISABLED for Frigate testing. Boot "
             "count: %d",
             boot_count);
    safe_mode_active = false; // FORCE DISABLED
    // In safe mode, we do NOT start the stable timer, so count remains high
    // User must manually fix via OTA/Web or hard reset might trigger stable
    // timer if they let it run? Actually, if we stay in Safe Mode stable, we
    // should eventually clear it? Let's say Safe Mode is stable if it runs for
    // 2 mins.

    stable_timer = xTimerCreate("diag_stable", pdMS_TO_TICKS(120000), pdFALSE,
                                NULL, stable_timer_callback);
    xTimerStart(stable_timer, 0);

    return ESP_FAIL; // Indicate Safe Mode
  } else {
    // Normal boot - start timer to clear count if we survive
    stable_timer = xTimerCreate("diag_stable", pdMS_TO_TICKS(60000), pdFALSE,
                                NULL, stable_timer_callback);
    xTimerStart(stable_timer, 0);
  }
...
bool sys_diag_is_safe_mode(void) { return safe_mode_active; }
...
void sys_diag_wdt_init(int timeout_sec) {
  ...
  // Prefer reconfigure first to avoid noisy internal "already initialized"
  // error logs.
...
void sys_diag_wdt_feed(void) { esp_task_wdt_reset(); }

void sys_diag_wdt_remove(void) { esp_task_wdt_delete(NULL); }

const char *sys_diag_get_reset_reason(void) { return reset_reason_str; }

int sys_diag_get_boot_count(void) { return boot_count; }

void sys_diag_report_status(void) {
  if (mqtt_ha_is_connected()) {
    char msg[128];
    snprintf(msg, sizeof(msg), "Boot: %s (Count: %d)", reset_reason_str,
             boot_count);
