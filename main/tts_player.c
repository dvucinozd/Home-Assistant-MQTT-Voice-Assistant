    ...
    while (1) {
        ...
        if (xQueueReceive(audio_queue, &chunk, portMAX_DELAY) == pdTRUE) {
            ...
            if (tts_buffer_pos + chunk.length < TTS_BUFFER_SIZE) {
                ...
                ESP_LOGD(TAG, "Buffered audio chunk: %d bytes (total: %d)", chunk.length, tts_buffer_pos);
            } else {
                ESP_LOGW(TAG, "TTS buffer full, chunk dropped");
            }
...
esp_err_t tts_player_init(void)
{
    ...
    BaseType_t ret = xTaskCreate(playback_task, "tts_playback", 8192, NULL, 5, &playback_task_handle);
    if (ret != pdPASS) {
