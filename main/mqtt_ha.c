 ...
 */
static void build_discovery_topic(const char *component, const char *entity_id, char *topic, size_t topic_len)
{
    snprintf(topic, topic_len, "%s/%s/%s/%s/config", DISCOVERY_PREFIX, component, DEVICE_ID, entity_id);
}
 ...
 */
static void build_state_topic(const char *entity_id, char *topic, size_t topic_len)
{
...
static void build_command_topic(const char *entity_id, char *topic, size_t topic_len)
{
 ...
 */
static esp_err_t publish_discovery(const char *component, const char *entity_id, cJSON *config)
{
    if (!mqtt_connected) {
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_DATA:
            ESP_LOGI(TAG, "MQTT message received: %.*s = %.*s",
            ...
            for (int i = 0; i < entity_count; i++) {
                ...
                if (strncmp(event->topic, topic_buf, event->topic_len) == 0 && entities[i].callback) {
...
esp_err_t mqtt_ha_init(const mqtt_ha_config_t *config)
{
    ...
    esp_mqtt_client_config_t mqtt_cfg = {
        ...
        .credentials.client_id = config->client_id ? config->client_id : DEVICE_ID,
    };
                                   ...
                                   mqtt_ha_command_callback_t callback)
{
    ...
    return publish_discovery("number", entity_id, config);
}
                                   ...
                                   const char *options, mqtt_ha_command_callback_t callback)
{
    ...
    return publish_discovery("select", entity_id, config);
}
