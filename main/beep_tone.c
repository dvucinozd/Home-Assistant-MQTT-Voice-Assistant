...
esp_err_t beep_tone_play(uint16_t frequency, uint16_t duration, uint8_t volume)
{
    ...
    ESP_LOGI(TAG, "Playing beep: %d Hz, %d ms, vol=%d%%", frequency, duration, volume);
    ...
    float amplitude = (volume / 100.0f) * 16000.0f; // Max amplitude for 16-bit audio
    ...
    esp_err_t ret = bsp_extra_codec_set_fs(BEEP_SAMPLE_RATE, 16, I2S_SLOT_MODE_MONO);
    if (ret != ESP_OK) {
